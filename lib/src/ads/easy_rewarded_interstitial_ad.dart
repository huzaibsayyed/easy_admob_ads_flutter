...
/// Controller for a Google AdMob Rewarded Interstitial ad.
///
/// ```dart
/// final ad = EasyRewardedInterstitialAd(onRewardEarned: (reward) => grantCoins(reward.amount));
/// await ad.loadAd();
/// // later
/// final result = await ad.showAd();
/// ```
class EasyRewardedInterstitialAd with AdRetryScheduler, AdCooldownTracker {
  static final Logger _logger = Logger('EasyRewardedInterstitialAd');
  ...
  EasyRewardedInterstitialAd({this.onStateChanged, this.onRewardEarned}) {
  ...
  Future<void> loadAd() async {
    ...
    if (!ads.adsEnabled) {
      _logger.fine('Ads are disabled globally. Rewarded interstitial ad will not load.');
      _setState(AdState.disabled);
      return;
    }

    if (_state == AdState.loading || _state == AdState.loaded) {
      _logger.fine('Rewarded interstitial ad already loading or loaded. Skipping new load request.');
      return;
    }
    ...
    if (!ads.hasConnectivity) {
      _logger.fine('No network connectivity. Rewarded interstitial ad will not load yet.');
    ...
    try {
      await RewardedInterstitialAd.load(
        adUnitId: ads.adUnitId(AdType.rewardedInterstitial),
        ...
        rewardedInterstitialAdLoadCallback: RewardedInterstitialAdLoadCallback(
          onAdLoaded: (ad) {
            ...
            _logger.info('Rewarded interstitial ad loaded.');
            ...
            ad.fullScreenContentCallback = FullScreenContentCallback(
              ...
              onAdFailedToShowFullScreenContent: (ad, error) {
                _logger.warning('Failed to show rewarded interstitial ad: ${error.message}');
                ad.dispose();
                _ad = null;
                _setState(AdState.error);
                scheduleRetry(config.retryDelay, loadAd);
              ...
              onAdShowedFullScreenContent: (ad) => _logger.fine('Rewarded interstitial ad shown.'),
              onAdImpression: (ad) => _logger.fine('Rewarded interstitial ad impression recorded.'),
            );
          },
          onAdFailedToLoad: (error) {
            _logger.warning('Rewarded interstitial ad failed to load: ${error.message}');
            AdConfigDiagnostics.check(error, adUnitId: ads.adUnitId(AdType.rewardedInterstitial), adType: 'Rewarded Interstitial');
            _setState(AdState.error);
            scheduleRetry(config.retryDelay, loadAd);
          },
        ),
  ...
  Future<AdResult> showAd() async {
    ...
    final cooldown = ads.config.rewardedInterstitialCooldown;
