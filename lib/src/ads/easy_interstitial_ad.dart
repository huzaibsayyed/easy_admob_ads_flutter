...
/// Controller for a Google AdMob Interstitial ad.
///
/// ```dart
/// final ad = EasyInterstitialAd(onStateChanged: (state) => print(state));
/// await ad.loadAd();
/// // later
/// final result = await ad.showAd();
/// ```
class EasyInterstitialAd with AdRetryScheduler, AdCooldownTracker {
  static final Logger _logger = Logger('EasyInterstitialAd');
  ...
  EasyInterstitialAd({this.onStateChanged}) {
  ...
  Future<void> loadAd() async {
    ...
    if (!ads.adsEnabled) {
      _logger.fine('Ads are disabled globally. Interstitial ad will not load.');
    ...
    if (!ads.hasConnectivity) {
      _logger.fine('No network connectivity. Interstitial ad will not load yet.');
    ...
    await InterstitialAd.load(
      adUnitId: ads.adUnitId(AdType.interstitial),
      ...
      adLoadCallback: InterstitialAdLoadCallback(
        onAdLoaded: (ad) {
          ...
          ad.fullScreenContentCallback = FullScreenContentCallback(
            ...
            onAdShowedFullScreenContent: (ad) => _logger.fine('Interstitial ad shown.'),
            onAdImpression: (ad) => _logger.fine('Interstitial ad impression recorded.'),
          );
        },
        onAdFailedToLoad: (error) {
          _logger.warning('Interstitial ad failed to load: ${error.message}');
          AdConfigDiagnostics.check(error, adUnitId: ads.adUnitId(AdType.interstitial), adType: 'Interstitial');
          _setState(AdState.error);
          scheduleRetry(config.retryDelay, loadAd);
        },
      ),
  ...
  Future<AdResult> showAd() async {
    ...
    final cooldown = ads.config.interstitialCooldown;
    ...
    try {
      ...
      // requested to display. Callers that navigate away right after awaiting this
      // must wait for the actual dismissal, otherwise disposing this controller
      // mid-display tears down the ad before markShownNow() ever runs.
